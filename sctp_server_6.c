...
int main(void) {
	...
	handle_error((sd = socket(PF_INET6, SOCK_STREAM, IPPROTO_SCTP)) == -1, "get ipv6 socket");
	...
	addr[1].sin6_family = AF_INET6;
	addr[1].sin6_port = htons(SERVER_PORT);
    handle_error(inet_pton(AF_INET6, SERVER_ADDR_6, &addr[1].sin6_addr) <= 0, "inet_pton addr 1");
	addr[2].sin6_family = AF_INET6;
	addr[2].sin6_port = htons(SERVER_PORT);
    handle_error(inet_pton(AF_INET6, SERVER_ADDR2_6, &addr[2].sin6_addr) <= 0, "inet_pton addr 2");
	handle_error(bind(sd, (struct sockaddr *)&addr[1], sizeof(struct sockaddr_in6)) < 0, "bind1")
